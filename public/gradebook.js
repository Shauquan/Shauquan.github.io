...
function fetchGradeData() {
    ...
    xhr.onreadystatechange = function(){
        ...
        if(xhr.readyState === xhr.DONE){
            ...
            // And then csll the function to update the HTML with our data
...
function populateGradebook(data) {
    // This function will take the fetched grade date data and populatevthe table
    console.log("Populating gradebook with data:", data);
    let tableElm = document.getElementById("gradebook"); // Get the gradebook table element
        data.forEach(function(assignment){ // For each row data we're passed in
            let row = document.createElement("tr"); // create a table row element
            let columns = []; // Handy place to stick the columns of information
            columns.name = document.createElement('td'); // The first column's table data will be the nmae
